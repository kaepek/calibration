...
Kalman_vn = CyKalman.KalmanJerk1D(alpha, theta_resolution_error, jerk_error, False, 2**14) #kalman.Kalman_Filter_1D(alpha, theta_resolution_error, jerk_error)

#alpha = 6
#theta_resolution_error = 0.01
#jerk_error = 0.0000002
Kalman_angle = CyKalman.KalmanJerk1D(alpha, theta_resolution_error, jerk_error, False, 2**14) #kalman.Kalman_Filter_1D(alpha, theta_resolution_error, jerk_error)

# read dataset argument
dataset_name = sys.argv[1] if len(sys.argv) > 1 else 0 
filename = 'calibration-data/%s/merged_capture_data.csv' % (dataset_name)
...
def bokeh_callback():
    ...
    if ( idx + 1 >= len_std_in):
        ...
        kalman_vn = Kalman_vn.get_kalman_vector()[0]
