...
def find_number_of_clusters(foldername):
    filename_in_old_analysis = "./calibration-data/%s/kmedoids_clustered_zero_crossing_channel_detections.inliers.analysis.json" % (foldername)
...
def load_inliers_files(foldername):
    folder_file_path = "./calibration-data/%s/zero_crossing_detections.channels.inliers.json" % (foldername)
...
def load_analysis_files(foldername):
    folder_file_path = "./calibration-data/%s/kmedoids_clustered_zero_crossing_channel_detections.inliers.analysis.json" % (foldername)
    with open(folder_file_path, "r") as fin:
        return json.loads(fin.read())
...
combination_report = Report(report_title, "./calibration-data/combination-report-%s", folders_description)

# add report header

text="""
<h1>Zero-crossing analysis for combined runs.</h1>
<p>Analysis for combined run folders: FOLDERS</p>
...
ideal_spacing = analyse.get_ideal_distance(n_poles) # in digital encoder steps out of 2^14
...
ordered_mean_lkv_tuple_list = analyse.mean_to_ordered_lkv(new_mean)
...
#final_report_filename "/calibration-data/combination-report-%s.html" 
# what do we want "/calibration-data/commutation_state_%s.cpp"
...
folder_desc_id = folder_desc_id.replace("./calibration-data/", "")
