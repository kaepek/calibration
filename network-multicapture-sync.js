 ...
 *    - run_id [string], a name for the merged collected data to be stored within the ./calibration-data/[run_id].jsonl file. e.g. 'august_20_experiment_1'.
...
const run_folder_location = `calibration-data/${args.run_id}`;
...
let debounce = 0;
/**
 * Function to save the retrieved file_data from both hosts and save it to a new jsonl file, to be saved at this location: calibration-data/[run_id].jsonl. Finally
...
function shutdown(args) {
    const out_data_location = `calibration-data/${args.run_id}/raw_capture_data.jsonl`;
    if (debounce < 1) {
